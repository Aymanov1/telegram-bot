...
public interface LinkedUserTargetRepository extends JpaRepository<LinkedUserTarget, Serializable> {
	...
	@Query("SELECT COUNT(u) FROM LinkedUserTarget u WHERE u.fromUser=:fromUser and u.sentInvitation=true")
	Long countJoinedInvitation(@Param("fromUser") String fromUser);
}
